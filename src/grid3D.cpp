...
Real Grid3D::Update_Grid(void)
{
  ...
  else if (H.nx > 1 && H.ny > 1 && H.nz > 1) //3D
  {
    ...
    #endif //not_VL
    #ifdef VL
    max_dti = VL_Algorithm_3D_CUDA(g0, g1, H.nx, H.ny, H.nz, x_off, y_off, z_off, H.n_ghost, H.dx, H.dy, H.dz, H.xbound, H.ybound, H.zbound, H.dt, H.n_fields, density_floor, U_floor, C.Grav_potential, max_dti_slow, &(H.t_data_wall));
    #endif //VL
    #ifdef SIMPLE
